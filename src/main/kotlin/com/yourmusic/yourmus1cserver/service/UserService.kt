...
class UserService (
    ...
    fun signup(requestDTO: SignupRequestDTO): User {
        ...
        if (userRepository.existsByUsername(requestDTO.username)){
            throw IllegalArgumentException("이미 존재하는 아아디입니다.");
        }
        val encodedPassword = passwordEncoder.encode(requestDTO.password)
        val user = User(username = requestDTO.username, password = encodedPassword, email = requestDTO.email, name = requestDTO.name)
